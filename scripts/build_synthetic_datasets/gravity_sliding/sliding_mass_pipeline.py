#!/usr/bin/env python3
"""
大规模 gravity_sliding 流水线：按批渲染 JPEG → 转 MP4 → 删除除第 1 帧外的图片，循环直到总样本数完成。
sample_id 从 0 连续递增（与 sliding_render.sh 一致）。

须在仓库/项目根目录下执行本脚本，以便 Blender 的 cwd 与 sliding_render 中 .cache 等相对路径一致。
"""
...
from sliding_jpg_to_mp4 import create_mp4_from_frames, glob_sample_dir  # noqa: E402
from sliding_prune_jpg_frames import prune_sample_dir  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch render → MP4 → prune frames for gravity_sliding"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="sliding_render 输出根目录（其下为 sample_<id>_g_*）",
    )
    parser.add_argument(
        "--blend",
        type=Path,
        default=Path("scripts/build_synthetic_datasets/gravity_sliding/sliding.blend"),
        help="相对当前工作目录的 .blend 路径（请在项目根目录执行）",
    )
    parser.add_argument(
        "--render-script",
        type=Path,
        default=Path("scripts/build_synthetic_datasets/gravity_sliding/sliding_render.py"),
        help="相对当前工作目录的 sliding_render.py（请在项目根目录执行）",
    )
    parser.add_argument(
        "--blender-bin",
