...
RENDER_DIR_ROOT=".cache/gravity_falling/jpgs"
NUM_GPUS=1
TOTAL_SAMPLES=1  
...
PYTHON_SCRIPT="scripts/build_synthetic_datasets/gravity_falling/falling_render.py"
...
while [ "$job_idx" -lt "$TOTAL_SAMPLES" ]; do
    ...
    # Blender 不会执行动画渲染。序列帧由 falling_render.py 内 bpy.ops.render.render(animation=True) 完成。
