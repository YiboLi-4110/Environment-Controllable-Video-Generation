...
class ControlNet_PatchEmbedding(torch.nn.Module):
    ...
    Input: (bs, 48, T, H/16, W/16)
    Output: (bs, T*H/32*W/32, 3072)
    """
    def __init__(self, in_channels=48, dim=3072, patch_size=(1, 2, 2)):
        super().__init__()

        # Wan2.2-TI2V-5B params: (48->3072, [1, 2, 2], [1, 2, 2])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.patch_embedding(x) # (bs, 48, T, H/16, W/16) --> (bs, 3072, T, H/32, W/32)
        x = rearrange(x, 'b c t h w -> b (t h w) c').contiguous() # (bs, T*H/32*W/32, 3072)
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None, controlnet=False, controlnet_num_layers=0, controlnet_stride=None):
        # height/width_division_factor are overwritten in from_pretrained by vae.upsampling_factor*2 (16 for I2V, 32 for TI2V-5B)
        super().__init__(
    ...
    def __call__(
        ...
        # Shape (480x832 = 480p; for 720p use height=704, width=1280)
...
class WanVideoUnit_ImageEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        ...
        us = pipe.vae.upsampling_factor
        msk = torch.ones(1, num_frames, height // us, width // us, device=pipe.device)
        msk[:, 1:] = 0
        ...
        msk = msk.view(1, msk.shape[1] // 4, 4, height // us, width // us)
...
class WanVideoUnit_FunControl(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, control_video, num_frames, height, width, tiled, tile_size, tile_stride, clip_feature, y, latents):
        ...
        if clip_feature is None or y is None:
            ...
            y = torch.zeros((1, y_dim, (num_frames - 1) // 4 + 1, height // us, width // us), dtype=pipe.torch_dtype, device=pipe.device)
        else:
            y = y[:, -y_dim:]
